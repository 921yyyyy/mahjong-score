...
let statsGrid = null; // Grid.js インスタンス保持用
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updatePlayerSuggestions = async () => {
        ...
        if (data) {
            ...
            if (datalist) datalist.innerHTML = [...new Set(data.map(p => p.name))].map(n => `<option value="${n}">`).join('');
        }
    };
    ...
    if (dbSubmitBtn) {
        dbSubmitBtn.onclick = async () => {
            // ... (GitHubのdbSubmitBtn.onclickの中身をそのまま配置。ただし「supabase.createClient」は不要)
            alert("保存しました。");
...
async function loadHomeSummary() {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<div class="col-span-3 text-slate-500 text-xs py-4">No data</div>';
        return;
    }
